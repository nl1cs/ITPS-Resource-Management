...
train_dataset_math = defaultdict(list)
    ...
    for col in df.columns[df.columns.str.startswith('subjects.')]:
        ...
        if pd.notna(subject):
            ...
            if subject in math_subjects:
                train_dataset_math[subject].append(row['title'])
            else:
                train_dataset_verbal[subject].append(row['title'])

# Convert the defaultdict to a regular dictionary
train_dataset_math = dict(math_subjects)

doc_bins = {}

db = DocBin()
categories = list(train_dataset_math.keys())

# Iterate through the categories and questions
for category, questions in train_dataset_math.items():
...
for subject, titles in train_dataset_math.items():
    for title in titles:
        titles.append(title)
